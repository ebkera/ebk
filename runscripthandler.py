...
class RunScriptHandler():
    ...
    def create_torque_job(self):
        with open (f"{self.identifier}.job1", "w+") as file:
            ...
            file.write(f"while read dir\n")
            file.write(f"do\n")
            file.write(f"    # Skip empty lines and comments\n")
            file.write(f'    [[ $dir == +(""|"#"*) ]] && continue\n')
            file.write(f"    # Let's use some basic shell variable string operations to remove the\n")
            file.write(f"    # leading and trailing parts of the dir names that are the same, isolating\n")
            file.write(f"    # the changing middle section as a useful short name.\n")
            file.write("    job_name=${dir#*KE+}\n")
            file.write("    job_name=${job_name%^a*}\n")
            file.write(f"    # Use another here-doc to read the job script, to obviate individual files\n")
            file.write(f"    # in each data directory.\n")
            file.write(f"\n")
            file.write(f"    # A here-doc with leading '-' will get leading TABs removed.  (unwise to\n")
            file.write(f"    # use, however, if your editor munges TABs.)\n")
            file.write(f"\n")
            file.write(f"    # Double-quote $dir to avoid parameter substitution.  (This is not strictly\n")
            file.write(f"    # necessary here, though, because the chars '^+' are not special -- in the\n")
            file.write(f"    # circumstances used here.)\n")
            file.write(f'    qsub -w "$PWD/$dir" -N "$job_name" <<-END_JOB_SCRIPT\n')
            file.write(f"\n")
            file.write(f"    #!/bin/bash\n")
            ...
            file.write(f"    #PBS -l walltime={self.walltime_hours}:{self.walltime_mins}:{self.walltime_secs}\n")
            ...
            file.write(f"    #PBS -e $PWD/$dir/job.err\n")
            file.write(f"    #PBS -j eo\n")
            file.write(f"    # Send mail at begin, end, abort, or never (b, e, a, n). Default is 'a'.\n")
            file.write(f"    #PBS -m bea\n")
            file.write("\n")
            file.write(f"    # change into the directory where qsub will be executed\n")
            file.write(f"    cd $PBS_O_WORKDIR\n\n")
            file.write(f"    # use a per-job lineup of modules; stern\n")
            file.write(f"    module purge\n")
            file.write(f"    module load intel\n")
            file.write(f"    module load openmpi/1.10/intel-17\n")
            file.write(f"    module load quantum-espresso/5.4/openmpi-1.10\n")
            file.write(f"    module list\n\n")
            ...
            file.write(f"    mpirun pw.x < run.in > run.out\n")
            file.write(f"END_JOB_SCRIPT\n")
            file.write(f"\n")
            file.write(f"done <<'END_TASKLIST'\n")
            file.write(f"    # Single quoting the limit string 'EOT' will pass strings without shell variable and execution expansion.\n")
            file.write(f"    # Comments and empty line are fine because we explicitly skip them.\n\n")
            for run in self.all_runs_list:
                file.write(f"    {run}\n")
            file.write(f"END_TASKLIST\n")
