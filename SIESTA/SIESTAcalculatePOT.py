...
class SIESTAcalculatePOT(SiestaReadOut):
    ...
    def calculate_pointcharge_coordinates(self, plot=False):
    ...
    def calculate_coc_quadrupole(self):
        ...
        for i,coordinates in enumerate(self.point_charge_coordinates):
            for row in range(0,3):
                for col in range(0,3):
                    if i == 0 or i == 3:
                        self.coc_positive_quadrupole[row,col] += self.q_p*((coordinates[row])*(coordinates[col]))/0.2081943  # also converting back to D.A
                    else:
                        self.coc_negative_quadrupole[row,col] += self.q_n*((coordinates[row])*(coordinates[col]))/0.2081943
    ...
    def get_pot_difference(self):
        ...
        f = 0.2081943*10**(-10)*10**(-10)  # factor for converting the quadurupole units to em^2 and not eA^2
