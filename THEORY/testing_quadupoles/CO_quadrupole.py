...
class OCCO_quadrupole():
    ...
    def set_positions_actual_ionic(self, CC_distance:float) -> None:
        self.point_charges = []  # will contain all the charges in this format: [[x,y,z], 1/-1] ([position],charge)
        self.point_charges.append([[0,0,-CC_distance/2],4])
        self.point_charges.append([[0,0,CC_distance/2],4])
        self.point_charges.append([[0,0,-CC_distance/2-self.CO_dist],6])
        self.point_charges.append([[0,0,CC_distance/2+self.CO_dist],6])
        ...
        for point_charge in self.point_charges:
            ...
            for col in range(0,3):
                for row in range(0,3):
                    ...
                    # if col == row:
                        # print("traceless row:", row, point_charge[1], (3*(point_charge[0][row])*(point_charge[0][col]) - r2*f))
                        # print("nontraceless :", row, point_charge[1], ((point_charge[0][row])*(point_charge[0][col])))
        # print(self.Q)
    ...
    def print_values_to_console(self):
        # Some testing prints
        print("\nCalculated values:\n")
        print("dipoles from siesta (DA, eA): ", self.CO_dipole_from_siesta,"\n", self.CO_dipole_in_eA)
        unit_factor_Debye = 1/0.2081943 #(same as c.elementary_charge*10**(-10)/(3.33564*10**(-30)))
        print(f"Q in       : \n{self.Q}")
        print(f"Q_non_traceless : \n{self.Q_non_traceless}")
        # print(f"Q in eA          : \n{self.Q}")
